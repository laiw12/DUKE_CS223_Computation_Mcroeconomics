...
## The efficiency is equal to all positive profits - all negative profits
maximize efficiency: sum{b in BIDDERS,e in POSITIVE_HYPEREDGES} positive_valuation[b,e]*positive_applies[b,e] -  sum{b in BIDDERS, e in NEGATIVE_HYPEREDGES} negative_valuation[b,e]*negative_applies[b,e];




## the item is only assigned to one bidder. 
...
## In the below constraint, if all items are revieved in e, negative_applies[b,e] is forced to set to 1 to meet the constraint. If exist some unreceived items, the statement is always
## true because the solver will set negative_applies to zero and there will be at least one item that is not assigned. Hence (>=1).
